...
def poison_with_influence_proj_gradient_step(model, indices_to_poison, grad_influence_wrt_input_val_subset, step_size,
                                             project_fn):
    """
    ...
    poisoned_X_train_subset = project_fn(
        model.data_sets.train.x[indices_to_poison, :] - step_size * np.sign(
            grad_influence_wrt_input_val_subset) * 2.0 / 255.0)
...
def iterative_attack(top_model, full_model, top_graph, full_graph, project_fn, test_indices, test_description=None,
    ...
    for attack_iter in range(num_iter):
        ...
        if len(test_indices) < 100:
            ...
            if ((early_stop is not None) and (len(test_indices) == 1)):
                if test_pred[0, int(full_model.data_sets.test.labels[test_indices])] < early_stop:
                    ...
                    np.savez('output/%s_attack_%s_testidx-%s_trainidx-%s_stepsize-%s_proj_final' % (
        ...
        if (attack_iter + 1) % save_iter == 0:
            np.savez('output/%s_attack_%s_testidx-%s_trainidx-%s_stepsize-%s_proj_iter-%s' % (
